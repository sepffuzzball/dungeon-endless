...
- `/play/[runId]`: view an owned active or finished run in the chronological expedition terminal. `?/act` resolves the current room, `?/proceed` explicitly enters the next room after narration and summary are durable, and `?/abandon` settles and closes an active run.
...
JavaScript-enhanced actions return one exact durable narration ID: acting streams only its turn, while proceeding streams only the new room. Ordinary HTML submissions finalize deterministic fallbacks before redirecting, so the resolved result and the separate Proceed Deeper form remain usable without JavaScript or an available model endpoint.
...
The run seed, room number, turn sequence, and purpose label seed every room, check, reward, and settlement stream. Complete charter metadata and the current room are persisted at run start, including a pending initial room-entry snapshot. Each accepted action inserts one immutable turn at `version + 1`, applies its authoritative HP and rewards, and moves the run to `awaiting_proceed` without changing rooms. Only an idempotent, UUID-keyed Proceed Deeper command generates and reveals the next room; it does not increment the turn version or resolve rewards. Expected versions reject stale forms, and row locks plus phase checks prevent concurrent actions or proceeds from advancing twice. Narration and summaries run outside gameplay transactions and cannot change immutable snapshots or outcomes.
