...
- `/play/[runId]`: view an owned active or finished run in the chronological expedition terminal. `?/act` resolves the current room, `?/searchLoot` awards deterministic post-encounter loot, `?/resolveFailure` narrates a settled failure once, `?/proceed` explicitly enters the next room after narration and summary are durable, and `?/abandon` settles and closes an active run.
...
- Starting-loot tiers 1 through 3 cost 25, 75, and 225 company gold. Every expedition starts with one common magic item; the three purchased tiers add one uncommon, rare, and very rare magic item respectively. Starting items are sellable and use an isolated deterministic stream, so they vary with the run seed without changing room generation.
...
- New rules-version-3 expeditions snapshot the character's level, stats, gear, starting room, and company settings and use depth-scaled loot rarity. Later edits and upgrades cannot change an active run. Rules-version-1/2 runs keep their saved charter data and retained legacy reward generation when an old encounter snapshot is missing a pool; any explicitly saved reward array, including an empty one, is immutable.
...
- Defense is `5 + effective Body + direct defense equipment`. Instinct is effective Body + effective Mind + effective Spirit, and Attack is Instinct + direct attack equipment; level has no separate Attack or Defense contribution. Attribute and general equipment affect Attack and Defense through effective attributes, whose equipment bonuses are capped at +5 per attribute. Skill, direct attack, and direct defense effects are uncapped, and multiple effects on one item stack independently.
...
- A failed monster, boss, or trap check costs 1 HP and requires one consequence narration before proceeding. Fatal settlement remains atomic with the failed action, while one final consequence narration remains available. A rest room requires no check and heals 1 HP. Common through very rare draughts heal 1 through 4 HP; excess healing increases maximum HP by that excess. Legendary and artifact draughts increase maximum HP by 4 and 5 respectively, then fully heal. Legacy draughts retain capped heal-one behavior.
- Treasure requires no check, has three generated rewards, and resolves one draw immediately. Successful monster and trap encounters reveal a deterministic three-item post-encounter pool; bosses reveal six and draw two. Every reward has one of six depth-scaled rarities: common, uncommon, rare, very rare, legendary, or artifact. Kind distribution remains equal thirds magic, draught, and valuable. Drawn draughts are recorded and consumed in draw order, while only non-draught rewards enter inventory.
- In common-to-artifact order, rarity weights are `70/25/5/0/0/0` at rooms 1-4, `55/30/12/3/0/0` at 5-9, `40/30/20/8/2/0` at 10-19, `25/30/25/14/5/1` at 20-34, `15/25/25/20/12/3` at 35-49, and `10/20/25/22/16/7` at room 50 and deeper.
- Magic sale dice by rarity are 1d2, 2d2, 2d4, 2d6, 3d8, and 4d10; valuable sale dice are 1d6 through 6d6. On defeat or abandonment, sellable inventory is settled exactly once in inventory order. A valid fixed value takes precedence and consumes no RNG, explicit dice are rolled once per die, unsellable items and draughts pay zero, and legacy magic/valuable items retain 1d2/1d6 fallback values. Abandoning at run version 0 excludes starting loot, preventing farm-and-abandon gold; after one resolved action it sells normally as a delving incentive.
...
Migration `0004` also requires a quiesced single-writer rollout: stop every application writer, apply migrations once from one deployment process, then start only the matching application build. Existing runs are initialized to the `ready` phase while the nullable room-entry command key and its per-run unique index are added for later proceed idempotency. Do not run old and new application writers concurrently during this cutover.

Migration `0005` requires the same quiesced single-writer rollout: stop all application writers, apply migrations once, then deploy only the matching application build. Before creating the run-wide `(run_id, action_key)` unique index, the migration checks for duplicate key groups and raises a clear exception without deleting data; operators must resolve any reported duplicates explicitly before retrying. The migration also extends the durable run phase enum for loot and nonfatal failure resolution. Do not run older writers concurrently with this schema.
